...
<section>
	...
	<div class="main_cover">
		...
		<a href="user_guide.do?fromMainMap=서울"> <span>이달의 추천 도시 '서울'로 떠나보세요</span></a>
	</div>
		...
		<div class="col-md-10">
			<div class="d-flex justify-content-between align-items-center" style="width:100%;">
				...
				<div class="total-button">
					<a href="editor_essay.do"><span class="view_toditor">전체보기</span></a>
				</div>
			</div>
					...
					<div class="col-md-4">
						<div class="card mb-4 box-shadow">
							...
							<div class="card-body" onclick="location.href='toditor_detail.do?seq=${item.seq }'" style="cursor: pointer;">
								...
								<span style="font-size: 12px; color: #f37720;">[${item.comment_count }]</span><br>
					...
					<div class="card mb-4 box-shadow">
						...
						<div class="card-body"
							...
							<span id="location">${item.category }</span><br> <span
								style="font-size: 15px; font-weight: bold;">${item.title }</span>&nbsp;<span
								style="font-size: 12px; color: #f37720;">[${item.comment_count }]</span><br>
							<span
 ...
 --%>
	...
	<div class="commu_part">
		...
		<div class="offset-md-9 move_to_" id="move_to_afterbbs">
			<a href="user_guid.do"><span class="view_toditor">전체보기</span></a>
		</div>
		<div class="2nd_best_content">

			<div class="row content col-md-10"
				style="border: 1px solid #dcdcdc; padding: 20px;">
				...
				<table class="col-md-6">
					...
					<tbody>
						<c:forEach items="${planlist }" begin="0" end="4" var="item"
							varStatus="i">
							<tr>
								<td>&nbsp;<a href="guideDetail.do?seq=${item.seq }">[${item.location }]&nbsp;${item.title }</a></td>
								<td><img src="${initParam.IMG_SERVER_PATH}/image/heart.png"
									width="30px" height="30px">${item.like_count }</td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
				...
				<table class="col-md-6">
					...
					<tbody>
						<c:forEach items="${planlist }" begin="5" end="9" var="item"
							varStatus="i">
							<tr>
								<td>&nbsp;<a href="guideDetail.do?seq=${item.seq }">[${item.location }]&nbsp;${item.title }</a></td>
								<td><img src="${initParam.IMG_SERVER_PATH}/image/heart.png"
					...
					<table class="commu-hot-table">
						...
						<tbody>
							<c:forEach items="${planlist }" begin="0" end="4" var="item" varStatus="i">
								<tr>
									<td>
										<a href="guideDetail.do?seq=${item.seq }">[${item.location }]&nbsp;${item.title }</a>
									</td>
									<td>
										<img src="${initParam.IMG_SERVER_PATH}/image/heart.png" width="30px" height="30px">${item.like_count }
									</td>
								</tr>
							</c:forEach>
						</tbody>
					...
					<table class="commu-hot-table">
						...
						<tbody>
							<c:forEach items="${commulist }" begin="0" end="4" var="item" varStatus="i">
								<tr>
									<td>
										<a href="afterdetail.do?seq=${item.seq }">${item.title }&nbsp;
											<span style="font-size: 12px; color: #f37720;">[${item.comment_count }]</span>
										</a>
									</td>
									<td>
										<img src="${initParam.IMG_SERVER_PATH}/image/heart.png" width="30px" height="30px">${item.like_count }
			...
			<div class="row no-gutters" style="border: 1px solid #dcdcdc; padding: 20px;">
				<div class="d-flex justify-content-between align-items-center" style="width:100%;">
					...
					<div class="downloadcount">
						<p>
							<strong>전체 다운로드 수</strong> &nbsp; <strong style="color: #7DC3BB;"><fmt:formatNumber value="${downcount }" type="number" /></strong>
						</p>
					</div>
				</div>
				...
				<div class="d-flex justify-content-around align-items-end" style="width:100%;padding:10px 0px;">
					<c:forEach items="${totolist }" var="toto" varStatus="i">
						<div class="book-card">
							<div class="book-img" onclick="location.href='toto_detail.do?seq=${toto.seq }'">
								<img class=book-img-top" src="${initParam.IMG_SERVER_PATH}/image/${toto.pic}" style="cursor: pointer; width: 100%; heigh: 100%;">
							</div>
